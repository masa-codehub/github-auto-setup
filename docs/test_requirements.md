...
# テスト要件: 開発環境2サーバー分離・E2E・運用 (DEV-ENV-REFACTOR-217)
- id: TR-ENV-001
  description: フロントエンド（静的HTML/JS）とバックエンド（Django/DRF APIサーバー）が完全に分離され、各々独立して起動・開発・デプロイできること。
- id: TR-ENV-002
  description: バックエンドAPIサーバーはCORS設定により、指定フロントエンド（例: http://localhost:3001）からのリクエストのみ許可されていること。
- id: TR-ENV-003
  description: 2サーバー構成でのE2Eテスト（ファイルアップロード→解析→Issue選択→GitHub登録/ローカル保存）が全ての主要ブラウザで正常に動作すること。
- id: TR-E2E-001
  description: フロントエンドの全API呼び出し（fetch等）は絶対URL（http://localhost:8000/api/...等）で行われ、環境変数や設定で切り替え可能であること。
- id: TR-E2E-002
  description: バックエンドAPIの全エンドポイントは、CORS/認証/エラーハンドリングを含めてE2Eテストで検証されていること。
- id: TR-E2E-003
  description: README等の運用手順通りに2サーバーを起動し、E2Eテストがパスすること。
