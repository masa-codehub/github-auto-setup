...
class AiSettings(BaseModel):
    ...
    openai_model_name: str = Field(
        "gpt-4o", description="Default OpenAI model name if not set by env var")
    gemini_model_name: str = Field(
        "gemini-1.5-flash", description="Default Gemini model name if not set by env var")
    prompt_template: str = Field(
        default="Default prompt template {markdown_text}",
        description="Prompt template for AI parser"
    )  # デフォルト値を提供
    separator_rule_prompt_template: str = Field(
        default="",
        description="区切りルール推論用プロンプトテンプレート"
    )
    key_mapping_rule_prompt_template: str = Field(
        default="",
        description="キーマッピングルール推論用プロンプトテンプレート"
    )
...
class Settings(BaseSettings):
    ...
    ai_model: str = Field(
        'openai', validation_alias='AI_MODEL')  # デフォルトは openai
    openai_api_key: Optional[SecretStr] = Field(
        None, validation_alias='OPENAI_API_KEY')
    gemini_api_key: Optional[SecretStr] = Field(
        None, validation_alias='GEMINI_API_KEY')
    # モデル名は環境変数でも上書き可能にする
    env_openai_model_name: Optional[str] = Field(
        None, validation_alias='OPENAI_MODEL_NAME')
    env_gemini_model_name: Optional[str] = Field(
        None, validation_alias='GEMINI_MODEL_NAME')
    # ログレベルも環境変数で上書き可能にする
    ...
    def final_log_level(self) -> str:
        ...
        if env_level is not None and env_level not in valid_levels:
            logger.warning(
                f"Invalid log level '{self.env_log_level}' in environment variable. Using YAML/default.")
        ...
        logger.warning(
            f"Invalid log level '{self.logging.log_level}' in config. Defaulting to INFO.")
...
class YamlConfigSettingsSource(PydanticBaseSettingsSource):
    ...
    def __call__(self) -> Dict[str, Any]:
        ...
        if not self.config_file_path.exists():
            logger.warning(
                f"YAML configuration file not found: {self.config_file_path}. Using defaults and environment variables.")
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded settings from YAML: {self.config_file_path}")
        except yaml.YAMLError as e:
            logger.error(
                f"Error parsing YAML file '{self.config_file_path}': {e}", exc_info=True)
            return {}  # パースエラー時も空の辞書を返す
        except IOError as e:
            logger.error(
                f"Error reading YAML file '{self.config_file_path}': {e}", exc_info=True)
            return {}  # 読み込みエラー時も空の辞書を返す
        except Exception as e:
            logger.error(
                f"Unexpected error loading YAML file '{self.config_file_path}': {e}", exc_info=True)
            return {}  # その他のエラー時も空の辞書を返す

        # プロンプトテンプレートが存在しない場合は警告ログを出力
        if 'ai' in yaml_data and not yaml_data['ai'].get('prompt_template'):
            logger.warning(
                "prompt_template is missing in ai section of YAML config. Using defaults.")
...
def load_settings(config_file: Path = Path("config.yaml")) -> Settings:
    ...
    logger.info(
        f"Attempting to load configuration from: {config_file.resolve()}")
    logger.info(
        f"Environment variables will override settings from {config_file.name}.")
    ...
    try:
        ...
        logger.debug(
            f"Config File  : {config_file.resolve() if config_file.exists() else 'Not Found'}")
        # Use final properties for logging effective values
        logger.debug(
            f"OpenAI Model : {settings.final_openai_model_name} (Source: {'Env' if settings.env_openai_model_name else 'YAML/Default'})")
        logger.debug(
            f"Gemini Model : {settings.final_gemini_model_name} (Source: {'Env' if settings.env_gemini_model_name else 'YAML/Default'})")
        logger.debug(
            f"Log Level    : {settings.final_log_level} (Source: {'Env' if settings.env_log_level else 'YAML/Default'})")
        logger.debug(
            f"GitHub PAT   : {'Set' if settings.github_pat else 'Not Set'}")
        logger.debug(
            f"OpenAI Key   : {'Set' if settings.openai_api_key else 'Not Set'}")
        logger.debug(
            f"Gemini Key   : {'Set' if settings.gemini_api_key else 'Not Set'}")
        # Prompt template is large, maybe log its presence/absence or length
        logger.debug(
            f"Prompt Templ.: {'Loaded from YAML' if settings.prompt_template else 'Not Loaded'}")
