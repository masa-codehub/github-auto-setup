...
from core_logic.adapters.github_utils import github_api_error_handler
from core_logic.domain.exceptions import GitHubClientError, GitHubResourceNotFoundError
...
class GitHubRestClient:
    ...
    # color, descを受け取っても無視
    def _create_label_context(self, owner: str, repo: str, label_name: str, **_) -> str:
        return f"creating label '{label_name}' in {owner}/{repo}"

    # state, per_pageを受け取っても無視
    def _list_milestones_context(self, owner: str, repo: str, **_) -> str:
        return f"listing milestones for {owner}/{repo}"

    # state, descを受け取っても無視
    def _create_milestone_context(self, owner: str, repo: str, title: str, **_) -> str:
        return f"creating milestone '{title}' in {owner}/{repo}"

    # body etc を受け取っても無視
    def _create_issue_context(self, owner: str, repo: str, title: str, **_) -> str:
    ...
    def create_repository(self, repo_name: str) -> Repository:
        ...
        if not response or not response.parsed_data:
            ...
            raise GitHubClientError(
                f"Repository creation for '{repo_name}' seemed successful but response data is missing.")
        # INFO -> DEBUG
        logger.debug(
            f"Successfully created repository: {response.parsed_data.html_url}")
    ...
    def get_repository(self, owner: str, repo: str):
        ...
        logger.debug(
            f"Successfully retrieved repository: {response.parsed_data.html_url}")
        return response.parsed_data
    ...
    def get_authenticated_user(self) -> User:
        ...
        if not response or not response.parsed_data or not response.parsed_data.login:
            raise GitHubClientError(
                "Could not retrieve valid authenticated user data from response.")
        logger.debug(
            f"Successfully retrieved authenticated user: {response.parsed_data.login}")
        return response.parsed_data
    ...
    def get_label(self, owner: str, repo: str, label_name: str) -> Optional[Label]:
        ...
        if response and response.status_code == 200 and not response.parsed_data:
            logger.warning(
                f"get_label for '{label_name}' returned 200 OK but no data.")
            return None
    ...
    def create_label(self, owner: str, repo: str, label_name: str,
        ...
        logger.info(
            f"Attempting to create label '{trimmed_label_name}' in {owner}/{repo}")
        ...
        if not response or not response.parsed_data:
            raise GitHubClientError(
                f"Label creation for '{trimmed_label_name}' seemed successful but response data is missing.")
        # INFO -> DEBUG
        logger.debug(f"Successfully created label '{trimmed_label_name}'.")
        return response.parsed_data
    ...
    def list_milestones(self, owner: str, repo: str, state: str = "open", per_page: int = 100) -> List[Milestone]:
        ...
        logger.debug(
            f"Listing {state} milestones for {owner}/{repo} (per_page={per_page})")
    ...
    def create_milestone(self, owner: str, repo: str, title: str,
        ...
        logger.info(
            f"Attempting to create milestone '{trimmed_title}' in {owner}/{repo}")
        ...
        if state not in ("open", "closed"):
            logger.warning(
                f"Invalid state '{state}' provided for milestone '{trimmed_title}', defaulting to 'open'.")
        ...
        if not response or not response.parsed_data or response.parsed_data.number is None:
            raise GitHubClientError(
                f"Milestone creation for '{trimmed_title}' seemed successful but response data is missing or invalid.")
        # INFO -> DEBUG
        logger.debug(
            f"Successfully created milestone '{trimmed_title}' with ID {response.parsed_data.number}.")
        return response.parsed_data
    ...
    def create_issue(self, owner: str, repo: str, title: str,
        ...
        trimmed_title = title.strip()
        logger.info(
            f"Attempting to create issue '{trimmed_title}' in {owner}/{repo}")
        payload: Dict[str, Any] = {"owner": owner,
                                   "repo": repo, "title": trimmed_title}
        if body is not None:
            payload["body"] = body
        # リストが渡された場合のみ、空要素を除去して設定
        if labels is not None:
            payload["labels"] = [lbl for lbl in labels if lbl and lbl.strip()]
        if milestone is not None:
            payload["milestone"] = milestone
        if assignees is not None:
            payload["assignees"] = [a for a in assignees if a and a.strip()]
        ...
        if not response or not response.parsed_data or not response.parsed_data.html_url:
            raise GitHubClientError(
                f"Issue creation for '{trimmed_title}' seemed successful but response data is missing.")
        # INFO -> DEBUG
        logger.debug(
            f"Successfully created issue '{trimmed_title}': {response.parsed_data.html_url}")
        return response.parsed_data
    ...
    def search_issues_and_pull_requests(self, q: str, per_page: int = 1) -> Any:
        ...
        """
        logger.debug(
            f"Searching issues/PRs with query: {q} (per_page={per_page})")
        ...
        if not response or not response.parsed_data or not hasattr(response.parsed_data, 'total_count') or response.parsed_data.total_count is None:
            raise GitHubClientError(
                f"Search for '{q}' seemed successful but response data is missing or invalid.")
        logger.debug(
            f"Search for '{q}' returned {response.parsed_data.total_count} total results.")
        return response.parsed_data
    ...
    def check_collaborator(self, owner: str, repo: str, username: str) -> bool:
        ...
        """
        logger.debug(
            f"Checking collaborator status for '{username}' on {owner}/{repo}")
        ...
        if response is None:
            logger.debug(
                f"User '{username}' is not a collaborator on {owner}/{repo} (404 Not Found)")
            return False

        # 204 No Content が返された場合はコラボレーター
        is_collaborator = response.status_code == 204
        logger.debug(
            f"User '{username}' is{'' if is_collaborator else ' not'} a collaborator on {owner}/{repo}")
