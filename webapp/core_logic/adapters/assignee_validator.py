...
class AssigneeValidator:
    ...
    def validate_assignees(self, owner: str, repo: str, assignee_logins: List[str]) -> Tuple[List[str], List[str]]:
        ...
        logger.info(
            f"Validating {len(unique_logins_to_check)} unique assignee(s) for {owner}/{repo}...")
        ...
        for login in unique_logins_to_check:
            ...
            try:
                ...
                # APIエラーの場合は例外を送出する
                is_valid = self.rest_client.check_collaborator(
                    owner, repo, login)

                if is_valid:
                    logger.debug(
                        f"Assignee '{login}' is valid for {owner}/{repo}.")
                    valid_assignees.append(login)
                else:
                    # check_collaborator が False を返した場合 (404 Not Found など)
                    logger.warning(
                        f"Assignee '{login}' is not a collaborator or could not be verified for {owner}/{repo}.")
                ...
                logger.warning(
                    f"API error validating assignee '{login}' for {owner}/{repo}: {type(e).__name__} - {e}. Marking as invalid.")
                invalid_assignees.append(login)
            except Exception as e:
                # その他の予期せぬエラー
                logger.error(
                    f"Unexpected error validating assignee '{login}' for {owner}/{repo}: {type(e).__name__} - {e}", exc_info=True)
        ...
        if invalid_assignees:
            logger.warning(
                f"Assignee validation finished. Valid: {len(valid_assignees)}/{total_checked}, Invalid/Unverified: {len(invalid_assignees)}/{total_checked}. Invalid list: {invalid_assignees}")
        else:
            logger.info(
                f"Assignee validation finished. All {len(valid_assignees)}/{total_checked} assignee(s) are valid for {owner}/{repo}.")
