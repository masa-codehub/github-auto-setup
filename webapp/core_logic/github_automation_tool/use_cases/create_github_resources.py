...
class CreateGitHubResourcesUseCase:
    ...
    def __init__(self,
        ...
        if not isinstance(graphql_client, GitHubGraphQLClient):
            raise TypeError("graphql_client must be an instance of GitHubGraphQLClient")
        if not isinstance(create_repo_uc, CreateRepositoryUseCase):
             raise TypeError("create_repo_uc must be an instance of CreateRepositoryUseCase")
        if not isinstance(create_issues_uc, CreateIssuesUseCase):
             raise TypeError("create_issues_uc must be an instance of CreateIssuesUseCase")
    ...
    def _get_owner_repo(self, repo_name_input: str) -> Tuple[str, str]:
        ...
        if '/' in repo_name_input:
            ...
            try:
                ...
                logger.error(f"Failed to get authenticated user: {e}", exc_info=True)
                raise GitHubAuthenticationError(f"Failed to get authenticated user due to API error: {e}", original_exception=e) from e
            except Exception as e: # 予期せぬエラーも捕捉
                 logger.error(f"Unexpected error getting authenticated user: {e}", exc_info=True)
                 raise GitHubAuthenticationError(f"Unexpected error getting authenticated user: {e}", original_exception=e) from e
        ...
        try:
            ...
            if parsed_data.issues:
                for issue in parsed_data.issues:
                    if issue.labels: unique_labels_in_file.update(lbl for lbl in issue.labels if lbl and lbl.strip())
            ...
            if total_labels > 0:
                 ...
                 for i, label_name in enumerate(sorted_labels):
                    ...
                    try:
                        ...
                        existing_label = self.rest_client.get_label(repo_owner, repo_name, label_name)
                        if existing_label:
                            logger.info(f"Label '{label_name}' already exists.")
                            skipped_labels_count += 1
                        else:
                            logger.info(f"Label '{label_name}' not found, creating...")
                            self.rest_client.create_label(repo_owner, repo_name, label_name)
            ...
            if result.failed_labels: logger.warning(log_label_summary + f" Failed labels: {[l[0] for l in result.failed_labels]}")
            else: logger.info(log_label_summary)
            ...
            if parsed_data.issues:
                for issue in parsed_data.issues:
                    if issue.milestone and issue.milestone.strip(): unique_milestones_in_file.add(issue.milestone.strip())
            ...
            if total_milestones > 0:
                ...
                for i, milestone_name in enumerate(sorted(unique_milestones_in_file)):
                    ...
                    try:
                        ...
                        existing_milestones = self.rest_client.list_milestones(repo_owner, repo_name, state="all")
                        ...
                        if found_milestone:
                            if found_milestone.number is None: raise GitHubClientError(f"Found milestone '{milestone_name}' but it has no ID.")
                            milestone_id = found_milestone.number
                            logger.info(f"Milestone '{milestone_name}' already exists with ID: {milestone_id}.")
                        else:
                            logger.info(f"Milestone '{milestone_name}' not found, creating it...")
                            new_milestone = self.rest_client.create_milestone(repo_owner, repo_name, milestone_name)
                            if not new_milestone or new_milestone.number is None: raise GitHubClientError(f"Milestone '{milestone_name}' creation failed.")
                            milestone_id = new_milestone.number
                            logger.info(f"Milestone '{milestone_name}' created successfully with ID: {milestone_id}.")
            ...
            if result.failed_milestones: logger.warning(log_milestone_summary + f" Failed milestones: {[m[0] for m in result.failed_milestones]}")
            else: logger.info(log_milestone_summary)
            ...
            if project_name:
                ...
                try:
                    ...
                    project_node_id = self.graphql_client.find_project_v2_node_id(repo_owner, project_name)
                    if project_node_id:
                        result.project_node_id = project_node_id
                        logger.info(f"Found Project V2 '{project_name}' with Node ID: {project_node_id}")
                    else:
                        logger.warning(f"Project V2 '{project_name}' not found. Skipping item addition.")
            ...
            if project_node_id and issue_result and issue_result.created_issue_details:
                ...
                for i, (issue_url, issue_node_id) in enumerate(issue_result.created_issue_details):
                    ...
                    try:
                        ...
                        item_id = self.graphql_client.add_item_to_project_v2(project_node_id, issue_node_id)
                ...
                if result.project_items_failed: logger.warning(log_proj_summary + f" Failed items: {[f[0] for f in result.project_items_failed]}")
                else: logger.info(log_proj_summary)
            ...
            logger.info("GitHub resource creation workflow completed successfully.")
