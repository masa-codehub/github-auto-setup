...
from core_logic.domain.exceptions import GitHubClientError, GitHubValidationError
...
PARSED_DATA_MIXED = ParsedRequirementData(
    issues=[ISSUE1_DATA, ISSUE2_DATA, ISSUE3_DATA])
PARSED_DATA_WITH_ERROR = ParsedRequirementData(
    issues=[ISSUE1_DATA, ISSUE4_DATA, ISSUE3_DATA])
PARSED_DATA_EMPTY_ISSUES = ParsedRequirementData(issues=[])
PARSED_DATA_WITH_ASSIGNEES = ParsedRequirementData(
    issues=[ISSUE_WITH_VALID_ASSIGNEES,
            ISSUE_WITH_INVALID_ASSIGNEES, ISSUE_WITH_AT_ASSIGNEES]
)
...
def test_execute_all_new_issues(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    with caplog.at_level(logging.INFO):  # INFOレベルのログをキャプチャ
        result = create_issues_use_case.execute(
            PARSED_DATA_ALL_NEW, TEST_OWNER, TEST_REPO)

    assert isinstance(result, CreateIssuesResult)
    assert result.created_issue_details == [
        ("url/1", "node_id_1"), ("url/3", "node_id_3")]
    ...
    mock_github_client.search_issues_and_pull_requests.assert_any_call(
        q=expected_query1, per_page=1)
    mock_github_client.search_issues_and_pull_requests.assert_any_call(
        q=expected_query3, per_page=1)

    assert mock_github_client.create_issue.call_count == 2
    mock_github_client.create_issue.assert_has_calls([
        call(owner=TEST_OWNER, repo=TEST_REPO, title=ISSUE1_DATA.title,
             body=ISSUE1_DATA.description, labels=None, milestone=None, assignees=[]),
        call(owner=TEST_OWNER, repo=TEST_REPO, title=ISSUE3_DATA.title,
             body=ISSUE3_DATA.description, labels=None, milestone=None, assignees=[])
    ])
    # ログの検証
...
def test_execute_all_existing_issues(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            PARSED_DATA_ALL_EXISTING, TEST_OWNER, TEST_REPO)
    ...
    mock_github_client.search_issues_and_pull_requests.assert_called_once_with(
...
def test_execute_mixed_issues(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            PARSED_DATA_MIXED, TEST_OWNER, TEST_REPO)
...
def test_execute_find_issue_api_error(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    mock_github_client.search_issues_and_pull_requests.side_effect = [
        mock_error, mock_search_result2, mock_search_result3]
    ...
    assert result.created_issue_details == [
        ("url/for_issue2", "node_for_issue2")]  # 2番目のIssueが作成される
    assert result.skipped_issue_titles == [
        ISSUE3_DATA.title]  # 3番目のIssueがスキップされる
    assert result.failed_issue_titles == [
        ISSUE1_DATA.title]  # 1番目のIssueは find で失敗
    ...
    assert f"Processing issue 1/3: '{ISSUE1_DATA.title}'" in caplog.text
    # エラーログ
    assert f"Failed to process issue '{ISSUE1_DATA.title}': GitHubClientError - Find API Error" in caplog.text
    assert f"Processing issue 2/3: '{ISSUE2_DATA.title}'" in caplog.text
...
def test_execute_create_issue_api_error(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    mock_github_client.create_issue.side_effect = [
        mock_issue1, mock_error, mock_issue3]

    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            PARSED_DATA_WITH_ERROR, TEST_OWNER, TEST_REPO)

    assert result.created_issue_details == [
        ("url/1", "node1"), ("url/3", "node3")]  # 1, 3番目は成功
    assert result.skipped_issue_titles == []
    ...
    # エラーログ
    assert f"Failed to process issue '{ISSUE4_DATA.title}': GitHubValidationError - Create API Error" in caplog.text
...
def test_execute_empty_issue_list(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            PARSED_DATA_EMPTY_ISSUES, TEST_OWNER, TEST_REPO)
...
def test_execute_with_assignees_validation(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, mock_assignee_validator: MagicMock, caplog):
    ...
    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            PARSED_DATA_WITH_ASSIGNEES, TEST_OWNER, TEST_REPO)

    # 結果の検証
    assert len(result.created_issue_details) == 3
    assert len(result.validation_failed_assignees) == 2
    # 無効な担当者情報が記録されていることを確認
    assert any(title == ISSUE_WITH_INVALID_ASSIGNEES.title for title,
               _ in result.validation_failed_assignees)
    assert any(title == ISSUE_WITH_AT_ASSIGNEES.title for title,
               _ in result.validation_failed_assignees)
...
def test_execute_with_unexpected_error(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    mock_github_client.search_issues_and_pull_requests.side_effect = [
        mock_search_result1, Exception("Unexpected server error"), mock_search_result3]
    mock_issue1 = create_mock_issue("url/1", "node_id_1")
    mock_issue3 = create_mock_issue("url/3", "node_id_3")
    mock_github_client.create_issue.side_effect = [mock_issue1, mock_issue3]
    ...
    assert len(result.errors) == 1
    # エラーメッセージに「Unexpected error」が含まれる
    assert "Unexpected error" in result.errors[0]
    # 具体的なエラー内容も含まれる
    assert "Exception - Unexpected server error" in result.errors[0]
...
def test_create_issue_returns_none_values(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    # カスタムテストデータを作成
    test_data = ParsedRequirementData(
        issues=[ISSUE1_DATA, ISSUE2_DATA, ISSUE3_DATA])

    with caplog.at_level(logging.INFO):
        result = create_issues_use_case.execute(
            test_data, TEST_OWNER, TEST_REPO)

    # 結果の検証
    assert len(result.created_issue_details) == 1  # 1番目のIssueのみ成功
    assert result.failed_issue_titles == [
        ISSUE2_DATA.title, ISSUE3_DATA.title]  # 2,3番目は失敗
...
def test_execute_with_milestone_mapping(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock):
    ...
    issue_with_milestone = IssueData(
        title="Issue with Milestone", description="Body", milestone="Sprint 1")
    ...
    # 実行
    result = create_issues_use_case.execute(
        parsed_data, TEST_OWNER, TEST_REPO, milestone_id_map)
...
def test_execute_with_missing_milestone_id(create_issues_use_case: CreateIssuesUseCase, mock_github_client: MagicMock, caplog):
    ...
    # マイルストーンを含むIssueデータを作成
    issue_with_milestone = IssueData(
        title="Issue with Unknown Milestone", description="Body", milestone="Unknown Sprint")
    parsed_data = ParsedRequirementData(issues=[issue_with_milestone])
