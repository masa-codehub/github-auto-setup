...
from github_automation_tool.adapters.assignee_validator import AssigneeValidator
...
def test_validate_assignees_whitespace_and_empty(mock_rest_client, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        valid, invalid = validator.validate_assignees("owner", "repo", ["", " ", None])
...
def test_validate_assignees_all_valid(mock_rest_client):
    ...
    validator = AssigneeValidator(mock_rest_client)
    valid, invalid = validator.validate_assignees("owner", "repo", ["user1", "@user2"])
    ...
    mock_rest_client.check_collaborator.assert_any_call("owner", "repo", "user1")
    mock_rest_client.check_collaborator.assert_any_call("owner", "repo", "user2")
...
def test_validate_assignees_all_invalid(mock_rest_client, caplog):
    ...
    with caplog.at_level(logging.WARNING):
        valid, invalid = validator.validate_assignees("owner", "repo", ["invalid1", "invalid2"])
...
def test_validate_assignees_mixed(mock_rest_client, caplog):
    ...
    validator = AssigneeValidator(mock_rest_client)
    with caplog.at_level(logging.WARNING):
        valid, invalid = validator.validate_assignees("owner", "repo", ["valid-user", "invalid-user"])
...
def test_validate_assignees_api_error(mock_rest_client, caplog):
    ...
    mock_rest_client.check_collaborator.side_effect = GitHubClientError("API Error")
    validator = AssigneeValidator(mock_rest_client)
    with caplog.at_level(logging.WARNING):
        valid, invalid = validator.validate_assignees("owner", "repo", ["user1"])
...
def test_validate_assignees_unexpected_error(mock_rest_client, caplog):
    """予期せぬエラーが発生した場合のテスト"""
    mock_rest_client.check_collaborator.side_effect = ValueError("Unexpected Error")
    validator = AssigneeValidator(mock_rest_client)
...
def test_validate_assignees_deduplication(mock_rest_client):
    ...
    mock_rest_client.check_collaborator.return_value = True
    validator = AssigneeValidator(mock_rest_client)
    valid, invalid = validator.validate_assignees("owner", "repo", ["user1", "user1", "@user1"])
    
    assert valid == ["user1"]  # 重複は除去され、@も削除される
    assert invalid == []
    # APIは1回だけ呼ばれる（重複は排除される）
    mock_rest_client.check_collaborator.assert_called_once_with("owner", "repo", "user1")