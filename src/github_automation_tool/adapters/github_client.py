...
from githubkit.versions.latest.models import Label, Issue, Milestone, Repository
from githubkit.exception import RequestError, RequestTimeout, RequestFailed
from pydantic import SecretStr

# 新しいエラーハンドリングユーティリティをインポート
...
class GitHubAppClient:
    ...
    def _create_repo_context(self, repo_name: str, *args, **kwargs) -> str:
        """リポジトリ作成時のコンテキスト文字列を生成"""
    ...
    def create_repository(self, repo_name: str) -> str:
        ...
        if response and response.parsed_data and hasattr(response.parsed_data, 'html_url') and response.parsed_data.html_url:
            ...
            # これは予期しない成功レスポンス
            logger.error(f"Could not get repository URL from successful API response.")
            raise GitHubClientError(
                "Could not retrieve repository URL after creation.")

    # --- ラベル関連 ---

    def _get_label_context(self, owner: str, repo: str, label_name: str, *args, **kwargs) -> str:
        """ラベル取得時のコンテキスト文字列を生成"""
        ...
        try:
            ...
            if status_code == 404:
                ...
                return None
                 ...
                 # 404以外のRequestFailedはエラーハンドラに任せる
                 raise
        except Exception as e:
             # その他の例外もエラーハンドラに任せる
             raise
    
    def _create_label_context(self, owner: str, repo: str, label_name: str, *args, **kwargs) -> str:
        """ラベル作成時のコンテキスト文字列を生成"""
        trimmed_name = label_name.strip() if label_name else ""
        return f"ensuring label '{trimmed_name}' in {owner}/{repo}"
    ...
    def create_label(self, owner: str, repo: str, label_name: str,
        ...
        logger.info(f"Ensuring label '{trimmed_label_name}' in {owner}/{repo}...")
        ...
        if response and response.status_code == 201:
            ...
            logger.error(f"Label creation API call returned unexpected status: {status_code}")
            raise GitHubClientError(f"Unexpected status ({status_code}) during label creation for '{trimmed_label_name}'.")

    # --- マイルストーン関連 ---

    def _find_milestone_context(self, owner: str, repo: str, title: str, *args, **kwargs) -> str:
        """マイルストーン検索時のコンテキスト文字列を生成"""
        state = kwargs.get("state", "open")
        trimmed_title = title.strip() if title else ""
        return f"searching for {state} milestone titled '{trimmed_title}' in {owner}/{repo}"

    @github_api_error_handler(_find_milestone_context)
    def find_milestone_by_title(self, owner: str, repo: str, title: str, state: str = "open") -> Milestone | None:
        ...
        logger.debug(f"Searching for {state} milestone titled '{trimmed_title}' in {owner}/{repo}")
    ...
    def _create_milestone_context(self, owner: str, repo: str, title: str, *args, **kwargs) -> str:
        """マイルストーン作成時のコンテキスト文字列を生成"""
        trimmed_title = title.strip() if title else ""
        return f"ensuring milestone '{trimmed_title}' in {owner}/{repo}"
    ...
    def create_milestone(self, owner: str, repo: str, title: str,
        ...
        trimmed_title = title.strip()
        logger.info(f"Ensuring milestone '{trimmed_title}' in {owner}/{repo}...")
        ...
        if response and response.status_code == 201 and response.parsed_data and hasattr(response.parsed_data, 'number') and response.parsed_data.number is not None:
            ...
            logger.error(f"Could not get valid milestone ID from API response (Status: {status_code}, Data: {parsed_data_info}).")
            raise GitHubClientError(
                f"Failed to create milestone '{trimmed_title}' or retrieve its ID (Status: {status_code}).")

    # --- Issue 関連 ---

    def _create_issue_context(self, owner: str, repo: str, title: str, *args, **kwargs) -> str:
        """Issue作成時のコンテキスト文字列を生成"""
        trimmed_title = title.strip() if title else ""
        return f"creating issue '{trimmed_title}' in {owner}/{repo}"
    ...
    def create_issue(self, owner: str, repo: str, title: str,
        ...
        trimmed_title = title.strip()
        logger.info(f"Attempting to create issue '{trimmed_title}' in {owner}/{repo} with labels: {labels}, milestone: {milestone}, assignees: {assignees}")
        ...
        if response and response.status_code == 201 and response.parsed_data:
            ...
            if issue_url and issue_node_id:
                ...
                logger.error(f"Could not retrieve issue URL or Node ID after creation.")
            ...
            logger.error(f"Failed to get issue URL from API response (Status: {status_code}, Data: {parsed_data_info}).")
            raise GitHubClientError(
                f"Could not retrieve issue URL after creation (Status: {status_code}).")
    
    def _find_issue_context(self, owner: str, repo: str, title: str, *args, **kwargs) -> str:
        """Issue検索時のコンテキスト文字列を生成"""
        trimmed_title = title.strip() if title else ""
        return f"searching for open issue titled '{trimmed_title}' in {owner}/{repo}"

    @github_api_error_handler(_find_issue_context)
    def find_issue_by_title(self, owner: str, repo: str, title: str) -> bool:
        ...
        trimmed_title = title.strip()
        logger.debug(f"Searching for open issue titled '{trimmed_title}' in {owner}/{repo}")
        ...
        if response and response.parsed_data and hasattr(response.parsed_data, 'total_count') and response.parsed_data.total_count is not None:
            ...
            logger.warning(f"Could not determine issue existence from search API response. Response status: {getattr(response, 'status_code', 'N/A')}")
            # この場合、見つからなかったとして扱うか、エラーとするか？ -> API異常なのでエラーが良い
            raise GitHubClientError(
                f"Unexpected response format from issue search API.")

    # --- Project V2 関連 ---

    def _find_project_context(self, owner: str, project_name: str, *args, **kwargs) -> str:
        """Project V2検索時のコンテキスト文字列を生成"""
        trimmed_owner = owner.strip() if owner else ""
        trimmed_name = project_name.strip() if project_name else ""
        return f"finding Project V2 Node ID for '{trimmed_name}' owned by '{trimmed_owner}' (client-side filter)"

    @github_api_error_handler(_find_project_context)
    def find_project_v2_node_id(self, owner: str, project_name: str) -> str | None:
        ...
        logger.info(f"Attempting to find Project V2 Node ID for '{trimmed_name}' owned by '{trimmed_owner}' (client-side filter)...")
    ...
    def _add_item_to_project_context(self, project_node_id: str, content_node_id: str, *args, **kwargs) -> str:
        """アイテムをプロジェクトに追加時のコンテキスト文字列を生成"""
        p_id = project_node_id.strip() if project_node_id else ""
        c_id = content_node_id.strip() if content_node_id else ""
        return f"adding item '{c_id}' to project '{p_id}'"

    @github_api_error_handler(_add_item_to_project_context)
    def add_item_to_project_v2(self, project_node_id: str, content_node_id: str) -> str | None:
        """
        指定されたコンテンツ(Issue, PR など)を Project V2 に追加します。
        """
        ...
        if not isinstance(response_dict, dict) or "data" not in response_dict:
            logger.error(f"GraphQL response missing 'data' field or invalid format")
            raise GitHubClientError(f"Invalid GraphQL response format")
        
        data = response_dict.get("data")
        if not data:
            logger.error(f"GraphQL response 'data' field is null or empty.")
            raise GitHubClientError(f"Missing data in GraphQL response")
        ...
        # 有効な項目IDが取得できない場合
        raise GitHubClientError(f"Failed to add item to project V2: Invalid response format or missing item ID")

    # --- 担当者検証 ---

    def _validate_assignees_context(self, owner: str, repo: str, assignee_logins: list[str], *args, **kwargs) -> str:
        """担当者検証時のコンテキスト文字列を生成"""
        return f"validating assignees for {owner}/{repo}: {assignee_logins}"

    @github_api_error_handler(_validate_assignees_context)
    ...
    # --- 廃止された古いエラーハンドリングメソッド ---
    # 新しいデコレータで置き換えるため、コメントアウトして残しておく
