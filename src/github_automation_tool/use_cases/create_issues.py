...
class CreateIssuesUseCase:
    ...
    def __init__(self, rest_client: GitHubRestClient, assignee_validator: AssigneeValidator):
        ...
        if not isinstance(assignee_validator, AssigneeValidator):
             raise TypeError("assignee_validator must be an instance of AssigneeValidator")
    ...
    def execute(self, parsed_data: ParsedRequirementData, owner: str, repo: str, 
        ...
        for i, issue_data in enumerate(parsed_data.issues):
            ...
            try:
                ...
                search_results = self.rest_client.search_issues_and_pull_requests(q=query, per_page=1)
                ...
                if exists:
                    ...
                    if issue_data.assignees:
                        ...
                        if invalid_assignees_for_issue:
                            logger.warning(f"Found {len(invalid_assignees_for_issue)} invalid assignee(s) for issue '{issue_title}': {invalid_assignees_for_issue}")
                            result.validation_failed_assignees.append((issue_title, invalid_assignees_for_issue))
                    ...
                    if created_issue and created_issue.html_url and created_issue.node_id:
                        result.created_issue_details.append((created_issue.html_url, created_issue.node_id))
                    else:
