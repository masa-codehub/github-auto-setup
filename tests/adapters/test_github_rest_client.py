...
def test_create_repository_with_empty_response(rest_client):
    ...
    assert "Repository creation for 'new-repo' seemed successful but response data is missing" in str(excinfo.value)
...
def test_get_authenticated_user_invalid_response(rest_client):
    ...
    assert "Could not retrieve valid authenticated user data" in str(excinfo.value)
...
def test_get_label_not_found(rest_client):
    ...
    result = rest_client.get_label(TARGET_OWNER, TARGET_REPO, "non-existent-label")
...
def test_get_label_success_but_no_data(rest_client, caplog):
    ...
    with caplog.at_level(logging.WARNING):
        result = rest_client.get_label(TARGET_OWNER, TARGET_REPO, "weird-label")
...
def test_create_label_trims_input(rest_client):
        ...
        rest_client.create_label(TARGET_OWNER, TARGET_REPO, "  trimmed-label  ", color="#ff00ff")
...
def test_create_label_handles_hash_in_color(rest_client):
    ...
    # #付きの色コードでメソッドを呼び出し
    rest_client.create_label(TARGET_OWNER, TARGET_REPO, "color-test", color="#00ff00")
...
def test_list_milestones_with_custom_params(rest_client):
    ...
    # カスタムパラメータでメソッドを呼び出し
    rest_client.list_milestones(TARGET_OWNER, TARGET_REPO, state="closed", per_page=50)
...
def test_create_milestone_api_error(rest_client):
    ...
    # メソッド呼び出しでエラーが発生することを確認
    with pytest.raises(GitHubValidationError) as excinfo:
        rest_client.create_milestone(TARGET_OWNER, TARGET_REPO, "Problematic Milestone")
...
def test_create_issue_api_error(rest_client):
    ...
    # メソッド呼び出しでエラーが発生することを確認
    with pytest.raises(GitHubAuthenticationError) as excinfo:
        rest_client.create_issue(TARGET_OWNER, TARGET_REPO, "No Permission Issue")
...
def test_check_collaborator_is_collaborator(rest_client):
    ...
    # メソッドを呼び出し
    result = rest_client.check_collaborator(TARGET_OWNER, TARGET_REPO, "collaborator")
