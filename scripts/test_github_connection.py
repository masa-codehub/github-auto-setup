...
app = typer.Typer(help="Test connection and authentication to the GitHub API using the configured PAT.")
...
def main():
    ...
    try:
        ...
        if not settings.github_pat or not settings.github_pat.get_secret_value():
            ...
            print("ERROR: GitHub PAT is missing. Please set GITHUB_PAT.", file=sys.stderr)
    ...
    try:
        ...
        print(f"\nERROR: Unexpected error during client initialization.", file=sys.stderr)
        all_checks_passed = False
    ...
    if github_client:
        try:
            ...
            if user_response and user_response.parsed_data and hasattr(user_response.parsed_data, 'login'):
                ...
                logger.info(f"Successfully retrieved authenticated user: {username}")
                print(f"Authenticated User: {username}")
            else:
                logger.error("Could not retrieve authenticated user login name from response.")
                print("ERROR: Failed to parse authenticated user response.", file=sys.stderr)
                all_checks_passed = False
            ...
            logger.error(f"Client error when getting user info: {e}", exc_info=True)
    ...
    if github_client:
        try:
            logger.info("Attempting to get rate limit info (includes scopes)...")
            rate_limit_response = github_client.gh.rest.rate_limit.get()

            if rate_limit_response and rate_limit_response.headers:
                scopes = rate_limit_response.headers.get("X-OAuth-Scopes", "N/A")
                limit = rate_limit_response.headers.get("X-RateLimit-Limit", "N/A")
                remaining = rate_limit_response.headers.get("X-RateLimit-Remaining", "N/A")
                reset_time = rate_limit_response.headers.get("X-RateLimit-Reset", "N/A")

                logger.info(f"Rate Limit Info: Limit={limit}, Remaining={remaining}, Reset={reset_time}")
                ...
                missing_scopes = [rs for rs in required_scopes if rs not in scopes_list]

                if missing_scopes:
                    logger.error(f"Missing required PAT scopes: {missing_scopes}")
                    print(f"ERROR: PAT is missing required scope(s): {', '.join(missing_scopes)}. Required: {', '.join(required_scopes)}", file=sys.stderr)
                    all_checks_passed = False
                else:
                    logger.info("Required scopes (repo, project) are present.")

            else:
                logger.error("Could not retrieve rate limit information or headers from response.")
                print("ERROR: Failed to parse rate limit response headers.", file=sys.stderr)
                all_checks_passed = False

        except GitHubClientError as e: # Catch errors during rate limit check
            logger.error(f"Client error when getting rate limit info: {e}", exc_info=True)
            print(f"\nERROR: API client error when getting rate limit info: {e}", file=sys.stderr)
            all_checks_passed = False
        except Exception as e:
            logger.error(f"Unexpected error getting rate limit info: {e}", exc_info=True)
            print(f"\nERROR: Unexpected error getting rate limit info.", file=sys.stderr)
            all_checks_passed = False
