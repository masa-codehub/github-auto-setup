...
app = typer.Typer(
    help="Test connection to the configured Generative AI model (OpenAI or Gemini).")


@app.command()
def main(
    ai_model_override: Annotated[str, typer.Option(
        "--ai-model", help="Override the AI model from settings ('openai' or 'gemini').")] = None,
):
    """
    ...
    try:
        ...
        if model_to_test == 'openai':
            if not settings.openai_api_key or not settings.openai_api_key.get_secret_value():
                logger.error(
                    "OpenAI API Key is required but not set in settings.")
                print(
                    "ERROR: OpenAI API Key is missing. Please set OPENAI_API_KEY.", file=sys.stderr)
            ...
            if not settings.gemini_api_key or not settings.gemini_api_key.get_secret_value():
                logger.error(
                    "Gemini API Key is required but not set in settings.")
                print(
                    "ERROR: Gemini API Key is missing. Please set GEMINI_API_KEY.", file=sys.stderr)
                raise typer.Exit(code=1)
            logger.info(f"Gemini API Key: Set")
        else:
            # This case should ideally not be reached if config validation is robust
            logger.warning(
                f"Unknown AI model selected: {model_to_test}. Proceeding, but client initialization might fail.")

    except Exception as e:
        logger.error(
            f"Failed to load settings or validate API key: {e}", exc_info=True)
    ...
    try:
        if model_to_test == "openai":
            ...
            logger.info(
                f"ChatOpenAI client initialized with model: {model_name}")
            ...
            logger.info(
                f"ChatGoogleGenerativeAI client initialized with model: {model_name}")

        else:
            raise ValueError(
                f"Unsupported AI model specified: '{model_to_test}'. Use 'openai' or 'gemini'.")
        ...
        logger.error(
            f"API Error during client initialization: {type(e).__name__} - {e}")
        print(
            f"ERROR: API Error during client initialization: {type(e).__name__}", file=sys.stderr)
        raise typer.Exit(code=1)
    except Exception as e:
        logger.error(
    ...
    if llm_client:
        test_prompt = "Hello!"
        logger.info(
            f"Attempting minimal API call with prompt: '{test_prompt}'")
        try:
            ...
            if response and hasattr(response, 'content'):
                ...
                if response_content and isinstance(response_content, str) and 'error' not in response_content.lower():
                    logger.info(
                        f"API call successful. Received response snippet: '{response_content[:50]}...'")
                    print(
                        f"SUCCESS: API connection test for '{model_to_test}' passed.")
                    print(
                        f"Response snippet: {response_content[:100]}{'...' if len(response_content) > 100 else ''}")
                else:
                    logger.error(
                        f"API call seemed successful but response content might indicate an issue: {response_content}")
                    print(
                        f"WARNING: API call completed for '{model_to_test}' but response content is suspicious. Please check logs.", file=sys.stderr)
                    # Treat suspicious content as failure for the test
                    raise typer.Exit(code=1)
            else:
                logger.error(
                    f"API call completed but response format was unexpected: {response}")
                print(
                    f"ERROR: API call completed for '{model_to_test}' but response format was unexpected.", file=sys.stderr)
                raise typer.Exit(code=1)

        except (*_OPENAI_ERRORS, *_GOOGLE_ERRORS) as e:
            # Log less verbosely for common API errors
            logger.error(
                f"API Error during test call: {type(e).__name__} - {e}", exc_info=False)
            print(
                f"ERROR: API call failed for '{model_to_test}'. Error: {type(e).__name__}", file=sys.stderr)
            print(
                f"Check your API key, permissions, and network connection.", file=sys.stderr)
            raise typer.Exit(code=1)
        except Exception as e:
            logger.error(
                f"Unexpected error during API call: {e}", exc_info=True)
            print(
                f"ERROR: Unexpected error during API call for '{model_to_test}'.", file=sys.stderr)
            raise typer.Exit(code=1)
    else:
